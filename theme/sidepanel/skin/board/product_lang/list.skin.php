...
if ($use_lang_filter && !empty($is_category) && empty($sca)) {
    $params = $_GET;
    $params['sca'] = $default_lang;
    unset($params['page']);
    ...
    $redirect_url = $path . ($query ? ('?' . $query) : '');
    echo '<script>location.replace(' . json_encode($redirect_url) . ');</script>';
    return;
}
